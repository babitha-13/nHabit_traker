...
void main() {
  group('BinaryTimeBonusHelper.scoreForLoggedMinutes (ON mode)', () {
    test('20m target, 10m logged -> 0.3333', () {
      ...
      expect(score, closeTo(0.333333333333, 1e-9));
    });

    test('20m target, 15m logged -> 0.5', () {
      final score = BinaryTimeBonusHelper.scoreForLoggedMinutes(
        loggedMinutes: 15,
        targetMinutes: 20,
        priority: 1,
    ...
    test('60m target, 120m logged -> 2.5', () {
      final score = BinaryTimeBonusHelper.scoreForLoggedMinutes(
        loggedMinutes: 120,
        targetMinutes: 60,
        priority: 1,
      ...
      expect(score, closeTo(2.5, 1e-3));
    });
  });

  group('BinaryTimeBonusHelper.scoreForLoggedMinutes (OFF mode)', () {
    test('20m target: 20->1.0, 40->1.7, 60->2.2', () {
      ...
      expect(score60, closeTo(2.2, 1e-9));
    });
  ...
  group('BinaryTimeBonusHelper.scoreForTargetMinutes', () {
    test('ON mode target score: 20->0.666, 60->1.7', () {
      ...
      expect(score20, closeTo(0.666666666666, 1e-9));
