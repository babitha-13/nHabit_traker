...
class CompletionCalendarWidget extends StatelessWidget {
  ...
  /// Dates that had partial progress even if final status is 'skipped'.
  /// Allows UI to show a different color for skipped-but-started days.
  final Set<DateTime> progressDates;

  const CompletionCalendarWidget({
    Key? key,
    required this.completionHistory,
    this.daysToShow = 90,
    this.progressDates = const {},
  }) : super(key: key);
  ...
  bool _hasProgress(DateTime date) {
    // Normalize to date-only to match keys callers likely provide
    final normalizedDate = DateTime(date.year, date.month, date.day);
    return progressDates.contains(normalizedDate);
  }
