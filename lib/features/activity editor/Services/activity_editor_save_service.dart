...
import 'package:habit_tracker/services/Activtity/start_date_change_dialog.dart';
...
class ActivityEditorSaveService {
  ...
  static Future<void> save(ActivityEditorDialogState state) async {
    ...
    if (!state.quickIsTaskRecurring && state.reminders.isNotEmpty) {
      final validationError =
          ActivityEditorReminderService.validateReminderTimes(state);
  ...
  static Future<void> createNewActivity(
    ...
    await createActivity(
      ...
      specificDays: state.quickIsTaskRecurring &&
          ...
          : null,
      startDate:
          state.quickIsTaskRecurring ? state.frequencyConfig?.startDate : null,
  ...
  static Future<void> updateExistingActivity(
      ActivityEditorDialogState state, CategoryRecord selectedCategory) async {
    final docRef = state.widget.activity!.reference;
    ...
    updateData['dueDate'] =
        (!state.quickIsTaskRecurring) ? state.dueDate : null;
    updateData['dueTime'] = state.selectedDueTime != null
        ? TimeUtils.timeOfDayToString(state.selectedDueTime!)
        : null;
    updateData['reminders'] = state.reminders.isNotEmpty
        ? ReminderConfigList.toMapList(state.reminders)
        : null;
