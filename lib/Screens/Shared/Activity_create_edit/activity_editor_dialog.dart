...
class _ActivityEditorDialogState extends State<ActivityEditorDialog> {
  static const String _createNewCategoryValue =
  ...
  void initState() {
    ...
    if (_isEssential) {
      ...
      _frequencyEnabled = t?.isRecurring ?? false;
      quickIsTaskRecurring = _frequencyEnabled;
      if (t == null) {
    ...
    try {
      if (_isEssential) {
        ...
        if (widget.activity == null) {
          await _createNewEssential(selectedCategory);
        } else {
          await _updateExistingEssential(selectedCategory);
        }
      } else {
  ...
  /// Helper to create frequency payload for essentials
  _EssentialFrequencyPayload _frequencyPayloadForEssential() {
    if (!_frequencyEnabled || _frequencyConfig == null) {
      return _EssentialFrequencyPayload(
        frequencyType: widget.activity != null ? '' : null,
        everyXValue: null,
        everyXPeriodType: null,
        specificDays: widget.activity != null ? <int>[] : null,
      );
    }
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    if (!_isEssential) _buildTaskTypeField(theme),
                    if (!_isEssential &&
                        (_selectedTrackingType == 'quantitative' ||
                            _selectedTrackingType == 'time')) ...[
                      const SizedBox(height: 12),
                    ...
                    if (!_isRecurring && !widget.isHabit && !_isEssential) ...[
                      const SizedBox(height: 12),
  ...
  Widget _buildFrequencyField(FlutterFlowTheme theme) {
    ...
    if (_isEssential) {
      ...
      displayText = _frequencyEnabled && _frequencyConfig != null
          ? _formatFrequencySummary()
          : "Manual only (won't auto-schedule)";
    } else {
      displayText = _isRecurring ? _formatFrequencySummary() : 'One-time task';
    }
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Expanded(
              child: Text(
                ...
                style: theme.bodyMedium.override(
                  color: (_isEssential && !_frequencyEnabled) ||
                          (!_isEssential && !_isRecurring)
                      ? theme.secondaryText
                      : theme.primaryText,
            ...
            if ((_isRecurring && !widget.isHabit && !_isEssential) ||
                (_isEssential && _frequencyEnabled))
              IconButton(
                icon: Icon(Icons.close, size: 18, color: theme.secondaryText),
                padding: EdgeInsets.zero,
                constraints: const BoxConstraints(),
