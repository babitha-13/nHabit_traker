...
class CumulativeScoreService {
  ...
  static Future<Map<String, dynamic>> updateCumulativeScore(
    ...
    try {
      ...
      final dailyScore =
          calculateDailyScore(todayCompletionPercentage, rawPointsEarned);
      ...
      if (todayCompletionPercentage < decayThreshold) {
        ...
        penalty = calculateCombinedPenalty(
  ...
  /// Returns structured data without UI formatting (separation of concerns)
  /// Use BonusNotificationFormatter in UI layer to format and display messages
  static Map<String, dynamic> getBonusNotificationData(
    Map<String, dynamic> scoreData,
  ) {
    // Return structured data only - no UI formatting
    // UI layer (BonusNotificationFormatter) will format messages
    return {
  ...
  static Future<Map<String, dynamic>> calculateProjectedDailyScore(
    ...
    try {
      ...
      if (todayCompletionPercentage < decayThreshold) {
        ...
        penalty = calculateCombinedPenalty(
            todayCompletionPercentage, projectedConsecutiveDays);
      } else {
