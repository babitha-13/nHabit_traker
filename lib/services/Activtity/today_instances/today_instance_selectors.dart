...
class TodayInstanceSelectors {
  ...
  static List<ActivityInstanceRecord> selectQueueItems(
    ...
    final pendingByTemplate = <String, List<ActivityInstanceRecord>>{};
    ...
    for (final instance in snapshot.instances) {
      ...
      final templateKey =
          '${instance.templateCategoryType}|${instance.templateId}';
      (pendingByTemplate[templateKey] ??= <ActivityInstanceRecord>[])
          .add(instance);
    }

    final selectedPending = <ActivityInstanceRecord>[];
    for (final entries in pendingByTemplate.values) {
      if (entries.isEmpty) continue;
      final itemType = entries.first.templateCategoryType;
