...
class InstanceEvents {
  ...
  static ActivityInstanceRecord createOptimisticProgressInstance(
    ...
    if (trackingType == 'quantitative' && currentValue != null && target != null) {
      final current = (currentValue is num) ? currentValue.toDouble() : 
                     (currentValue is String) ? double.tryParse(currentValue) ?? 0.0 : 0.0;
      final targetValue = (target is num) ? target.toDouble() : 
                         (target is String) ? double.tryParse(target) ?? 0.0 : 0.0;
      
      if (targetValue > 0 && current >= targetValue && original.status != 'completed') {
        updatedData['status'] = 'completed';
        updatedData['completedAt'] = now;
      }
    } else if (trackingType == 'time' && accumulatedTime != null && target != null) {
      final targetMs = ((target is num ? target.toDouble() : 
                        (target is String ? double.tryParse(target) ?? 0.0 : 0.0)) * 60000).toInt();
      
      if (targetMs > 0 && accumulatedTime >= targetMs && original.status != 'completed') {
        updatedData['status'] = 'completed';
        updatedData['completedAt'] = now;
      }
    }
