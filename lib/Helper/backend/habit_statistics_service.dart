...
class HabitStatisticsService {
  ...
  static Future<HabitStatistics> getHabitStatistics(
    ...
    if (normalizedEnd != null && todayNormalized.isAtSameMomentAs(normalizedEnd) ||
        normalizedEnd == null) {
      // Check if today's record exists
      final todayRecordExists = records.any((r) => 
        r.date != null && 
        DateTime(r.date!.year, r.date!.month, r.date!.day).isAtSameMomentAs(todayNormalized)
      );
      includeToday = !todayRecordExists && 
        (normalizedStart == null || !todayNormalized.isBefore(normalizedStart));
    }
    ...
    if (includeToday) {
      try {
        final todayData = await _getTodayDataFromInstances(userId, habitName, todayNormalized);
        if (todayData != null) {
    ...
    if (trackingType == 'time' || trackingType == 'quantity') {
      ...
      try {
        final completedInstances = await _getCompletedInstancesForHourDistribution(
          userId,
          habitName,
          normalizedStart,
          normalizedEnd,
    ...
    final averageSessionDuration = totalSessions > 0
        ...
        : const Duration();
    final averageQuantityPerCompletion = totalCompletions > 0
        ? totalQuantityCompleted / totalCompletions
        : 0.0;
    
    final completionsByDayOfWeek = _calculateCompletionsByDayOfWeekFromHistory(dailyHistory);
    final bestDayOfWeek = _calculateBestDayOfWeek(completionsByDayOfWeek);
    
    final weeklyBreakdown = _calculateWeeklyBreakdownFromHistory(dailyHistory, normalizedStart, normalizedEnd);
    final monthlyBreakdown = _calculateMonthlyBreakdownFromHistory(dailyHistory, normalizedStart, normalizedEnd);
  ...
  static Future<List<ActivityInstanceRecord>> _getCompletedInstancesForHourDistribution(
    String userId,
    String habitName,
    DateTime? startDate,
    DateTime? endDate,
  ...
  static Duration _calculateTotalTimeSpent(List<ActivityInstanceRecord> instances) {
  ...
  static int _calculateTotalSessions(List<ActivityInstanceRecord> instances) {
    ...
    for (final instance in instances) {
      ...
      } else if (instance.hasTotalTimeLogged() && instance.totalTimeLogged > 0) {
  ...
  static double _calculateTotalQuantityCompleted(List<ActivityInstanceRecord> instances) {
    double total = 0.0;
    for (final instance in instances) {
  ...
  static DateTime? _calculateBestMonth(List<Map<String, dynamic>> monthlyBreakdown) {
