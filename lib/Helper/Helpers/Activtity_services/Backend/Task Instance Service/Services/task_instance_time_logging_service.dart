...
class TaskInstanceTimeLoggingService {
  ...
  static Future<void> stopTimeLogging({
    ...
    try {
      ...
      final sessions = List<Map<String, dynamic>>.from(instance.timeLogSessions)..add(newSession);
      ...
      final optimisticData = Map<String, dynamic>.from(instance.snapshotData)
        ...
        ..['totalTimeLogged'] = totalTime
        ..['accumulatedTime'] = totalTime
        ..['currentValue'] = instance.templateTrackingType == 'time' ? totalTime : instance.currentValue
        ..['isTimeLogging'] = false
        ..['currentSessionStartTime'] = null
        ..['lastUpdated'] = now;
      if (markComplete) {
      ...
      try {
        ...
        final updatedInstance = await ActivityInstanceRecord.getDocumentOnce(activityInstanceRef);
  ...
  static Future<List<ActivityInstanceRecord>> getTimeLoggedTasks({
    ...
    try {
      if (startDate != null && endDate != null) {
        final normalizedStartDate = DateService.normalizeToStartOfDay(startDate);
        final normalizedEndDate = DateService.normalizeToStartOfDay(endDate);
        if (normalizedStartDate.add(const Duration(days: 1)).isAtSameMomentAs(normalizedEndDate)) {
          return getTimeLoggedTasksForDate(userId: uid, date: normalizedStartDate);
        }
      }
      final query = ActivityInstanceRecord.collectionForUser(uid).where('totalTimeLogged', isGreaterThan: 0);
      ...
      if (startDate != null || endDate != null) {
        ...
        return tasks.where((task) {
          ...
          return sessions.any((session) {
            ...
            final normalizedSessionStart = DateService.normalizeToStartOfDay(sessionStart);
            if (normalizedStartDate != null &&
                normalizedSessionStart.isBefore(normalizedStartDate)) {
              return false;
            }
            if (normalizedEndDate != null && !normalizedSessionStart.isBefore(normalizedEndDate)) {
              return false;
            }
            return true;
          });
    ...
    try {
          ...
          try {
            ...
            return dateInstances.where((instance) {
              ...
              return sessions.any((session) {
                final sessionStart = session['startTime'] as DateTime;
                final normalizedSessionStart = DateService.normalizeToStartOfDay(sessionStart);
                return normalizedSessionStart.isAtSameMomentAs(normalizedStartDate);
              });
            }).toList();
          } catch (e) {
      ...
      if (startDate != null || endDate != null) {
        final normalizedStartDate = startDate != null
            ? DateService.normalizeToStartOfDay(startDate)
            : null;
        final normalizedEndDate = endDate != null ? DateService.normalizeToStartOfDay(endDate) : null;
  ...
  static Future<void> logManualTimeEntry({
    ...
    try {
      if (templateId != null) {
        ...
        if (activityType == 'habit') {
          try {
            ...
            final habitInstances = await ActivityInstanceService.getHabitInstancesForDate(
          ...
          final tasks = await TaskInstanceTaskService.getTodaysTaskInstances(userId: uid);
        ...
        if (targetInstanceRef != null && existingInstance != null) {
          final currentSessions = List<Map<String, dynamic>>.from(existingInstance.timeLogSessions);
          ...
          try {
            ...
            final updatedInstance = await ActivityInstanceRecord.getDocumentOnce(targetInstanceRef);
            OptimisticOperationTracker.reconcileOperation(operationId, updatedInstance);
        ...
        if (activityType == 'habit') {
          throw Exception('Habit instance not found for the selected date. Please ensure the habit is active and appears in your habits list. New habit instances cannot be created from the time log.');
        }
        ...
        if (templateDoc.exists) {
          ...
          final instanceRef =
              await ActivityInstanceService.createActivityInstance(
                  ...
                  dueDate: instanceDueDate, // Preserve no due date if template doesn't have one
                  template: template,
                  userId: uid);
          ...
          final optimisticInstance = InstanceEvents.createOptimisticProgressInstance(
            createdInstance, accumulatedTime: totalTime,
          ...
          if (markComplete) {
            final optimisticData = Map<String, dynamic>.from(optimisticInstance.snapshotData);
            optimisticData['status'] = 'completed';
          ...
          InstanceEvents.broadcastInstanceUpdatedOptimistic(finalOptimisticInstance, operationId);
          final updateData = <String, dynamic>{
            'timeLogSessions': sessions,
            'totalTimeLogged': totalTime,
            'accumulatedTime': totalTime,
          ...
          try {
            ...
            final updatedInstance = await ActivityInstanceRecord.getDocumentOnce(instanceRef);
            OptimisticOperationTracker.reconcileOperation(operationId, updatedInstance);
          } catch (e) {
            OptimisticOperationTracker.rollbackOperation(operationId);
            rethrow;
      ...
      final taskInstanceRef = await TaskInstanceTimerTaskService.createTimerTaskInstance(
        userId: uid,
        startTimer: false,
        showInFloatingTimer: false,
      );
      final now = DateTime.now();
      final currentInstance = await ActivityInstanceRecord.getDocumentOnce(taskInstanceRef);
      final isessential = activityType == 'essential';
      final timeLogSessions = [newSession];
      if (isessential) {
        final templates = await essentialService.getessentialTemplates(userId: uid);
        ...
        final optimisticData = Map<String, dynamic>.from(currentInstance.snapshotData);
        optimisticData['status'] = markComplete ? 'completed' : 'pending';
        optimisticData['completedAt'] = markComplete ? endTime : FieldValue.delete();
        ...
        optimisticData['templateTrackingType'] = matchingTemplate?.trackingType ?? 'binary';
        ...
        try {
          ...
          final updatedInstance = await ActivityInstanceRecord.getDocumentOnce(taskInstanceRef);
          OptimisticOperationTracker.reconcileOperation(operationId, updatedInstance);
        } catch (e) {
          OptimisticOperationTracker.rollbackOperation(operationId);
          rethrow;
        ...
        final templateRef = ActivityRecord.collectionForUser(uid).doc(currentInstance.templateId);
        final optimisticData = Map<String, dynamic>.from(currentInstance.snapshotData);
        optimisticData['status'] = markComplete ? 'completed' : 'pending';
        optimisticData['completedAt'] = markComplete ? endTime : FieldValue.delete();
        optimisticData['isTimerActive'] = false;
        ...
        optimisticData['totalTimeLogged'] = totalTime;
        optimisticData['accumulatedTime'] = totalTime;
        optimisticData['currentValue'] = markComplete ? 1 : 0; // Binary one-offs: 1 if complete
        optimisticData['templateTarget'] = totalTime / 60000.0; // Minutes
        optimisticData['templateName'] = taskName;
        optimisticData['templateCategoryType'] = 'task';
        optimisticData['templateTrackingType'] = 'binary'; // Force binary for one-offs per previous request
        optimisticData['lastUpdated'] = now;
        optimisticData['currentSessionStartTime'] = null;
        ...
        final updateData = <String, dynamic>{
          ...
          'totalTimeLogged': totalTime,
          'accumulatedTime': totalTime,
          'currentValue': markComplete ? 1 : 0, // Binary one-offs: 1 if complete
  ...
  static Future<void> updateTimeLogSession({
    ...
    final uid = userId ?? TaskInstanceHelperService.getCurrentUserId();
    try {
      final instanceRef = ActivityInstanceRecord.collectionForUser(uid).doc(instanceId);
  ...
  static Future<bool> deleteTimeLogSession({
    ...
    try {
      ...
      try {
        ...
        if (instance.status == 'completed' && instance.templateTrackingType == 'quantitative' && instance.templateTarget != null) {
          final target = instance.templateTarget;
          if (target is num && target > 0) {
            final currentQty = (newCurrentValue is num) ? newCurrentValue.toDouble() : 0.0;
