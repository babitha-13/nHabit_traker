...
class ActivityInstanceCompletionService {
  ...
  static Future<StackedSessionTimes> calculateStackedStartTime({
    ...
    for (final item in simultaneous) {
      if (item.timeLogSessions.isNotEmpty) {
        ...
        final sessionDuration = lastSession['durationMilliseconds'] as int? ?? 0;
        ...
        final itemEffectiveEstimate = await TimeEstimateResolver.getEffectiveEstimateMinutes(
          userId: userId,
  ...
  static Future<void> completeInstanceWithBackdate({
    ...
    try {
      ...
      final existingSessions = List<Map<String, dynamic>>.from(instance.timeLogSessions);
      ...
      final effectiveEstimateMinutes = await TimeEstimateResolver.getEffectiveEstimateMinutes(
        userId: uid,
