...
class ActivityInstanceSchedulingService {
  ...
  static Future<void> skipInstance({
    ...
    bool skipAutoGeneration = false, // NEW: Prevent automatic next instance creation
  }) async {
      ...
      try {
        ...
        if (!skipAutoGeneration) {
          if (instance.templateCategoryType == 'habit') {
            await ActivityInstanceHelperService.generateNextHabitInstance(instance, uid);
          } else {
            ...
            if (templateDoc.exists) {
              ...
              if (template.isRecurring && template.isActive) {
                final recurrenceAnchorDate = instance.originalDueDate ?? instance.dueDate!;
                final nextDueDate = RecurrenceCalculator.calculateNextDueDate(
                  currentDueDate: recurrenceAnchorDate,
                  template: template,
                );
                if (nextDueDate != null) {
                  final newInstanceRef = await ActivityInstanceCreationService.createActivityInstance(
                  ...
                  try {
                    final newInstance = await ActivityInstanceHelperService.getUpdatedInstance(
        ...
        final updatedInstance = await ActivityInstanceHelperService.getUpdatedInstance(instanceId: instanceId, userId: uid);
  ...
  static Future<void> batchSkipInstances({
    ...
    final templateFutures = templateIds.map((templateId) async {
      final templateRef = ActivityRecord.collectionForUser(userId).doc(templateId);
      final templateDoc = await templateRef.get();
      if (templateDoc.exists) {
        final template = ActivityRecord.fromSnapshot(templateDoc);
    ...
    final nextInstanceFutures = instances.where((instance) =>
      ...
      try {
        ...
        final nextInstanceRef = ActivityInstanceRecord.collectionForUser(userId).doc();
    ...
    for (int batchStart = 0; batchStart < instances.length || nextInstanceIndex < nextInstanceRefs.length; batchStart += maxBatchSize) {
      ...
      while (nextInstanceIndex < nextInstanceRefs.length && operationsInBatch < maxBatchSize) {
        batch.set(nextInstanceRefs[nextInstanceIndex], nextInstanceDataList[nextInstanceIndex]);
        nextInstanceIndex++;
        operationsInBatch++;
      }
    ...
    final reminderFutures = instances.map((instance) async {
      try {
        await ReminderScheduler.cancelReminderForInstance(instance.reference.id);
      } catch (e) {
  ...
  static Future<DocumentReference?> bulkSkipExpiredInstancesWithBatches({
    ...
    try {
      ...
      if (instancesToSkipIndices.isNotEmpty) {
        ...
        for (int batchStart = 0;
          ...
          for (int j = batchStart; j < end; j++) {
            ...
            final windowDuration = await ActivityInstanceHelperService.calculateAdaptiveWindowDuration(
              template: template,
              userId: userId,
              currentDate: dueDate,
            );
            ...
            final newDocRef = ActivityInstanceRecord.collectionForUser(userId).doc();
      ...
      if (yesterdayInstanceIndex >= 0) {
        ...
        pendingInstanceRef = await ActivityInstanceCreationService.createActivityInstance(
          templateId: template.reference.id,
  ...
  static Future<void> removeDueDateFromInstance({
    ...
    try {
      ...
      final optimisticInstance = InstanceEvents.createOptimisticPropertyUpdateInstance(instance, {'dueDate': null},);
      final operationId = OptimisticOperationTracker.generateOperationId();
      OptimisticOperationTracker.trackOperation(
        operationId,
        instanceId: instanceId,
  ...
  static Future<void> skipInstancesUntil({
    ...
    try {
      ...
      final oldestInstance = ActivityInstanceRecord.fromSnapshot(oldestInstances.docs.first);
